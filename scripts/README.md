...
This script fetches the list of available models from the Academic Cloud API and generates a markdown report comparing them with the models configured in the extension.

### Prerequisites

You need an Academic Cloud API key. Set it as an environment variable:
...
# Or directly
ACADEMICCLOUD_API_KEY="your-api-key" npx tsx scripts/check-upstream-models.ts
```
...
2. **Models Only in Upstream** - Models available in Academic Cloud but not in the extension (with full JSON details for each)
...
3. Update `src/academiccloud.ts` with new model entries
